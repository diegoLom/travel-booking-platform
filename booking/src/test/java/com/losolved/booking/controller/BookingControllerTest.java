...
public class BookingControllerTest {
	...
	public void givenARetrieveBookingObject() throws Exception {
		...
		mockMvc.perform(get("/book/".concat(retrievedBooking.getId().toString()))).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.bookingDate",
						Matchers.equalTo(DateTimeFormatter.ISO_LOCAL_DATE_TIME.format(bookingDateTime))))
	...
	public void givenABookingOperation() throws Exception {
		...
		given(bookingService.book(ArgumentMatchers.any()))
				.willReturn(ResponseDTO.builder().code(201).message("Booking schedule").build());
		final LocalDateTime bookingDateTime = LocalDateTime.now().plusDays(2);
		...
		mockMvc.perform(post("/book").contentType(MediaType.APPLICATION_JSON).characterEncoding("utf-8").content(json)). // .accept(MediaType.APPLICATION_JSON)).//
	...
	public void givenAReviewBooking() throws Exception {
		...
		given(bookingService.reviewBooking(ArgumentMatchers.any()))
				.willReturn(ResponseDTO.builder().code(200).message("Booking reviewd").build());
		final LocalDateTime bookingDateTime = LocalDateTime.now().plusDays(2);
		BookingDTO bodyDTO = BookingDTO.builder().id(1l).accommodationId(1).bookingDate(bookingDateTime).build();
		String json = mapper.writeValueAsString(bodyDTO);

		mockMvc.perform(put("/book").contentType(MediaType.APPLICATION_JSON).content(json)).andExpect(status().isOk())
				.andExpect(jsonPath("$.code", Matchers.equalTo(200)))
	...
	public void givenABookingReviewNotFound() throws Exception {
		...
		mockMvc.perform(put("/book").contentType(MediaType.APPLICATION_JSON).content(json))
				...
				.andExpect(jsonPath("$.code", Matchers.equalTo(HttpStatus.NOT_FOUND.value())))
				.andExpect(jsonPath("$.message", Matchers.equalTo("Booking not found")));
	}

	@Test
	public void givenAnUnBooking() throws Exception {
		given(bookingService.undoBooking(ArgumentMatchers.any()))
				.willReturn(ResponseDTO.builder().code(200).message("Booking canceled").build());
